...
public class EventServiceImpl implements EventService {
	...
	public void insertEvent(Map<String, Object> map, HttpServletRequest request) throws Exception {
		...
		EventDAO.insertFile(Filelist.get(0));
		...
		Iterator<String> iterator = multipartHttpServletRequest
				.getFileNames();/*
								 * // 이터레이터를 이용하여 request에 전송된 모든 name을 이용한다 => Map에 있는 데이터를 while문을 이용하여 순차적으로
								 * 접근함
								 */
		MultipartFile multipartFile = null;
		while (iterator.hasNext()) {
			/*
			 * // hasNext()메서드는 Iterator내에 그 다음 데이터의 존재 유무를 알려주고 , Next()메서드는 Iterator내의
			 * 데이터를 // 가져온후, 커서를 다음위치로 이동시킨다
			 */
			 ...
			 * 그것이 Iterator.next() 메서드이다
			 */ if (multipartFile.isEmpty() == false) {/*
														 * // 실제 파일 정보가 있는지 검사한후에 아래의 메서드를 통해 파일의 정보를 출력한다(log.debug)
														 */
