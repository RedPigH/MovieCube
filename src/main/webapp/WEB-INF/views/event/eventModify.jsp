...
<body>
		...
		<table>
			...
			<tr>
					...
					<td><input type="text" class="txt w200" id="EVENT_NAME"
						...
						type="hidden" id="EVENT_NO" name="EVENT_NO"
						value="${map.EVENT_NO}" /> <font color="blue"><span
							class="ibk">예) 이벤트 제목 </span></font></td>
				</tr>
					...
					<td><select name="EVENT_TYPE" class="slct w200"
						value="${map.EVENT_TYPE}">
							<option value="무비큐브 이벤트"
								<c:if test="${map.EVENT_TYPE == '무비큐브 이벤트'}"> selected</c:if>>무비큐브
								이벤트</option>
							<option value="영화 이벤트"
								<c:if test="${map.EVENT_TYPE == '영화 이벤트'}"> selected</c:if>>영화
								이벤트</option>
							<option value="영화관 이벤트"
								<c:if test="${map.EVENT_TYPE == '영화관 이벤트'}"> selected</c:if>>영화관
								이벤트</option>
							<option value="제휴 이벤트"
								<c:if test="${map.EVENT_TYPE == '제휴 이벤트'}"> selected</c:if>>제휴
								이벤트</option>
					</select></td>
				</tr>
			...
			<tr>
					...
					<td><c:set var="TextValue" value="${map.EVENT_OPENDATE}" /> <input
						type="date" class="txt w200" id="EVENT_OPENDATE"
									...
									<tbody>
										...
										<tr>
											...
											<td><input type="hidden" id="IDX"
												name="IDX_ORGNAME_${var.index}" value="${row.FILE_NO}">
												<a href="#this" id="name" name="name"><font color="red">${row.EVENT_ORGNAME}</font>&nbsp;&nbsp;&nbsp;</a>
												<font color="blue"><span class="ibk">파일이 이미
														등록되어 있습니다. 새로 등록하시면 기존 파일이 삭제됩니다.</span></font> <input type="file"
												class="txt" id="EVENT_ORGNAME_${var.index}"
												name="EVENT_ORGNAME_${var.index}" /> <a href="#this"
												class="btn btnC_04 btnP_04" id="addFile"> <span>스틸컷
														추가</span></a> <a href="this" class="btn btnC_04 btnP_04" id="delete"
	...
	<div class="btn_type_03">
		<a href="#this" class="btn btnC_04 btnP_04" id="write"> <input
			type="hidden" name="MOVIE_NO" value="${param.EVENT_NO}" /><span>수정하기</span>
